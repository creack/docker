...
func (daemon *Daemon) Exec(config *runconfig.Config) (*Container, []string, error) {
	return nil, nil, fmt.Errorf("Unsupported")

	// Lookup container
	container := daemon.Get(config.Image)
	...
	_, err := daemon.execDriver.Exec(container.command[jobId], container.command[0].Process.Pid, pipes)
...
func (daemon *Daemon) Run(c *Container, pipes *execdriver.Pipes, startCallback execdriver.StartCallback) (int, error) {
	return daemon.execDriver.Run(c.command[0], pipes, startCallback)
}

func (daemon *Daemon) Kill(c *Container, sig int) error {
	return daemon.execDriver.Kill(c.command[0], sig)
}
