...
func (srv *Server) ContainerCreate(job *engine.Job) engine.Status {
	...
	if !config.NetworkDisabled && len(config.Dns) == 0 && len(srv.daemon.Config().Dns) == 0 && utils.CheckLocalDns(resolvConf) {
...
func NewServer(eng *engine.Engine, config *daemonconfig.Config) (*Server, error) {
	d, err := daemon.NewDaemon(config, eng)
	if err != nil {
		return nil, err
	}
	srv := &Server{
