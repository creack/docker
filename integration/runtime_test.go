...
func nuke(d *daemon.Daemon) error {
	return d.Nuke()
}
...
func init() {
	...
	// Create the "global daemon" with a long-running daemon for integration tests
	spawnGlobalDaemon()
...
func GetTestImage(d *daemon.Daemon) *image.Image {
	imgs, err := d.Graph().Map()
	if err != nil {
	...
	log.Fatalf("Test image %v not found in %s: %s", unitTestImageID, d.Graph().Root, imgs)
	return nil
}
